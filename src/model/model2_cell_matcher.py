...
class CellMatcher:
    ...
    def _select_cluster_count(self, features_scaled: np.ndarray) -> tuple[int, float | None]:
        ...
        return (1, None) if best_k == 1 else (best_k, best_score)
    ...
    def fit(self, df: pd.DataFrame, snapshot_cycle: int | None = None) -> "CellMatcher":
        ...
        features_scaled = self.scaler.fit_transform(df_clean[["IR_ohm", "OCV_V"]].to_numpy())
        selected_clusters, silhouette = self._select_cluster_count(features_scaled)
        self.kmeans = KMeans(n_clusters=selected_clusters, random_state=42, n_init=10).fit(features_scaled)
        self.metadata = {
            ...
            "compatibility_thresholds": {"IR_ohm": float(self.compatibility_ir_threshold), "OCV_V": float(self.compatibility_ocv_threshold)},
            "iqr_bounds": {"IR_ohm": self.ir_bounds, "OCV_V": self.ocv_bounds},
        }
        return self
    ...
    def _is_outlier(self, ir_ohm: float | None, ocv_v: float | None) -> bool:
        if ir_ohm is None or ocv_v is None or self.ir_bounds is None or self.ocv_bounds is None:
            return True
        return not (self.ir_bounds[0] <= ir_ohm <= self.ir_bounds[1] and self.ocv_bounds[0] <= ocv_v <= self.ocv_bounds[1])
    ...
    def predict(self, cells: list[dict]) -> list[dict]:
        ...
        for cell in cells:
            ir_ohm = float(cell.get("IR_ohm")) if cell.get("IR_ohm") is not None else None
            ocv_v = float(cell.get("OCV_V")) if cell.get("OCV_V") is not None else None
            ...
            result = {
                ...
                "notes": ["Outside training distribution or missing required measurements."] if outlier else [],
            }
        ...
        for result in results:
            if result["cluster"] is None:
                continue
            peers = [other["cell_id"] for other in results if other["cell_id"] != result["cell_id"] and self._cells_are_compatible(result, other)]
            result["compatible_with"] = peers
            result["notes"].append("Matched with at least one peer inside the compatibility thresholds." if peers else "Inside the training distribution, but no close peer match was found.")
        return results
